...
export const JurnalGuruView: React.FC<JurnalGuruProps> = ({
  ...
  const [inputTpDesc, setInputTpDesc] = useState<string>('');
  ...
  React.useEffect(() => {
    if (availableTpOptions.length > 0) {
      setInputTpDesc(availableTpOptions[0].label);
    } else {
      setInputTpDesc('TP 1 - Memahami konsep dasar');
    }
  }, [availableTpOptions]);
  ...
  const handleSimpanJurnal = (e: React.FormEvent) => {
    ...
    const newJurnal: JurnalRecord = {
      ...
      tujuanPembelajaran: inputTpDesc,
          ...
          <div className="space-y-4 text-xs">
            <div>
              <label className="block font-semibold text-slate-600 dark:text-slate-400 mb-1">
                Tujuan Pembelajaran (Dropdown dari TP Data Mapel)
              </label>
              <select
                value={inputTpDesc}
                onChange={(e) => setInputTpDesc(e.target.value)}
                className="w-full px-3.5 py-2.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-medium focus:ring-2 focus:ring-violet-500"
              >
              ...
              >
                ...
                <div className="text-xs text-slate-600 dark:text-slate-400 space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex items-center justify-between text-[11px] pt-1">
                    ...
                    <span className="font-bold text-emerald-600">{j.jumlahHadir} Hadir</span>
                  </div>
                </div>
              </div>
            ))}
